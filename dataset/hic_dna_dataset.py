...
class HiC_DNA_Dataset(Dataset):
            ...
            for i in range(L, R, stride):
                ...
                if not self.whole_row:
                    for j in range(i, R, stride):
                        ...
                        if j - i + (chunk - stride) >= self.max_separation:
                            break
                else:
                    row = []
                    ...
                    wanted_len = self.pos_per_row if self.pos_per_row > 0 else np.ceil(
                        (self.max_separation - (chunk - stride)) / stride
                    ).astype(int) + 1
    ...
    def read_hic(self, loaders: List[Union[HiC_Loader, Cool_Loader]], position, seed=0):
        ...
        for loader in loaders:
            ...
            if self.input_option == 'LC':
                assert self.target_option in ['hic', 'norm', 'hic+norm']
            ...
            if self.augment_resolution is not None:
                ...
                target_submatrix = np.sum(
                    np.reshape(target_submatrix, (-1, d0, D, d1, D)),
                    axis=(-1, -3)
                )

                d0, d1 = input_submatrix.shape[-1] // D, input_submatrix.shape[-2] // D
                input_submatrix = np.sum(
                    np.reshape(input_submatrix, (-1, d0, D, d1, D)),
                    axis=(-1, -3)
                )
        ...
        return (
            target_matrix.astype(np.float32),
            np.log(target_read_count).astype(np.float32),
            input_matrix.astype(np.float32),
            np.log(input_read_count).astype(np.float32)
        )
    ...
    def __getitem__(self, index):
        ...
        for position in positions:
            ...
            target_matrix, target_read_count, input_matrix, input_read_count = self.read_hic(
                [self.HiC_loaders[hi] for hi in hic_indices],
                (chr0, s0, e0, strand0, chr1, s1, e1, strand1),
                self.seed + index
            )
...
def prepare_datasets(
    ...
    **kwargs
    ...
    if target_option in ['hic', 'norm', 'hic+norm']:
        if target_specified is None:
            hic_loaders = create_loaders_with_index(
                'train',
                species,
                resolution=augment_resolution,
                high_coverage_hic=high_coverage_hic,
                read_count=read_count,
                hic_norm_type=hic_norm_type
            )
        else:
